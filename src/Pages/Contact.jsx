        ...
        <>
            ...
            <div className="container">
               ...
               <div className="flex justify-center mt-3">
                ...
                <div className="w-full">
                        {ContactDetails.inputs.map((items,index)=>(
                            <div key={index} className="w-full">
                                <input type="text" placeholder={items.detail} className="text-lg font-Heading text-black/70 bg-gray-200 placeholder:text-black/70 placeholder:font-Heading font-thin placeholder:text-lg w-full outline-none px-3 py-2"/>
                                <p className="text-red-500 text-[15px] font-light opacity-100">please enter {items.detail}</p>
                            </div>
                        ))}
                        <div>
                            <select name="budget" className="text-lg font-Heading text-black/70 bg-gray-200 font-thin w-full outline-none px-3 py-2">
                                <option value="">Select budget</option>
                                <option value="US $1000 to US $2500">US $1000 to US $2500</option>
                                <option value="US $2500 to US $5000">US $2500 to US $5000</option>
                                <option value="US $5000 to US $10000">US $5000 to US $10000</option>
                                <option value="US $10000 to US $25000">US $10000 to US $25000</option>
                                <option value="US $2500 & above">US $2500 & above</option>
                            </select>
                        </div>
                    </div>
                    <div className="w-full">
                            ...
                            <div>
                            {
                                ContactDetails.checkbox.map((items,index)=>(
                                    <div key={index} className="flex items-center py-2">
                                        ...
                                        <h3 className="text-[18px] my-auto font-thin text-black/90 inline ml-3">{items.service}</h3>
                                    </div>
                                ))
                            }
                        </div>
                ...
                <div>
                    <textarea name="" id="" className="text-xl bg-gray-200 text-black/70 placeholder:text-black/70 placeholder:font-Heading font-thin placeholder:text-lg w-full outline-none px-3 py-2" placeholder="Project details" rows={4}></textarea>
                </div>
                <div className="flex justify-center">
